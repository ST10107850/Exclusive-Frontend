...
export const CheckOut = () => {
  ...
  // Ensure cart.items is defined and is an array before performing reduce
...
const handlePlaceOrder = async () => {
  ...
  console.log("Cart Items: ", cart); // Log the cart ID to ensure it's being passed correctly
  console.log("Cart ID: ", cart?._id);

  // Make sure cartId is present before proceeding
  if (!cart?._id) {
  ...
  const orderData = {
    ...
    deliveryOption,
    cartId: cart?._id, // Make sure the cartId is not undefined or null
    totalAmount: deliveryFee + subtotal + tax, // Ensure this is a number, not a string
    taxAmount: tax, // Ensure this is a number, not a string
    orderStatus: "Processing", // Add the order status
  };

  console.log("Order Data before sending:", orderData);

  try {
    const order = await createOrder(orderData); // Send orderData to backend
    if (order) {
